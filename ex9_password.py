...
for u_pass in unique_passwords:
    ...
    if response.cookies != None:
        autorize_response = requests.post("https://playground.learnqa.ru/ajax/api/check_auth_cookie", data=payload, cookies=cookies)
        print(autorize_response.text)
        cookie_value = autorize_response.cookies.get('auth_cookie')
        print('--------------')
        if autorize_response.text != "You are NOT authorized":
            print(u_pass)
            print(autorize_response.text)
            break